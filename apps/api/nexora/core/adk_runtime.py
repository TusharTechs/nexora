...
def _configure_genai_env() -> None:
    ...
    NEXORA's own code reads GEMINI_API_KEY via os.getenv; google-genai reads it
    too. On the Vertex path a visible key + GOOGLE_GENAI_USE_VERTEXAI makes
    google-genai use Vertex "express" mode against the *key's* project (wrong
    project). So on the Vertex path we stash the key in a NEXORA-private var and
    remove both public names from google-genai's view.
    """
    if os.getenv("GEMINI_API_KEY") and not os.getenv("NEXORA_GEMINI_API_KEY"):
        os.environ["NEXORA_GEMINI_API_KEY"] = os.environ["GEMINI_API_KEY"]

    backend = os.getenv("NEXORA_LLM_BACKEND", "auto")
    project = os.getenv("GCP_PROJECT_ID", "")
    prefer_vertex = bool(project) and (
        backend == "vertex" or (backend == "auto" and not os.getenv("NEXORA_GEMINI_API_KEY")))
    if prefer_vertex:
        ...
        return
    key = os.getenv("NEXORA_GEMINI_API_KEY", "")
    if key:
